...
public class KeyboardCameraControl : MonoBehaviour
{
	...
	void LateUpdate()
	{
        ...
        {
            //sunAnimator.SetTrigger(Animator.StringToHash("rotate_sun"));
        }

        if (Input.GetAxis("Mouse ScrollWheel") != 0)
	    {
	        speed += Input.GetAxis("Mouse ScrollWheel")*speedSensitivity*speed;
	    }
		...
		if (horizontalTranslation.isActivated())
		{
            ...
            if (translateX != 0)
            {
                rb.AddForce(translateX * transform.right * horizontalTranslation.sensitivity * speed);
            }
        }
		if (depthTranslation.isActivated())
		{
            ...
            if (translateZ != 0)
            {
                rb.AddForce(translateZ * transform.forward * depthTranslation.sensitivity * speed);
            }
        }
